...
% arabika
% membaca file citra
nama_folder = 'data latih/arabika';
...
% robusta
% membaca file citra
nama_folder = 'data latih/robusta';
% membaca nama file yang berekstensi .jpg
nama_file = dir(fullfile(nama_folder,'*.jpg'));
% membaca jumlah file
jumlah_file = numel(nama_file);
...
ciri_latih = [ciri_arabika;ciri_robusta];
target_latih = [target_arabika;target_robusta];

% melakukan operasi transpose terhadap variabel ciri_latih dan target_latih

X = ciri_latih';
T = ind2vec(target_latih');

% %menetapkan nilai nilai parameter jaringan lvq
hiddenSize = 10;
lvqLR = 0.01;
numEpochs = 10;
lvqLF = 'learnlv1';
...
jaringan = lvqnet(hiddenSize,lvqLR,lvqLF);
jaringan.trainParam.epochs = numEpochs;

% %melakukan pelatihan jaringan 
jaringan = train(jaringan,X,T);

% %membaca nilai keluaran hasil pelatihan
Y = sim(jaringan,X);
hasil_latih = vec2ind(Y);

%menghitung akurasi pelatihan
 
jumlah_benar = 0;
jumlah_data = numel(hasil_latih);
for k = 1:jumlah_data
